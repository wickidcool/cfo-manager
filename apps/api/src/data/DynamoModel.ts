...
import { removeGSIFields } from '../utils/common/helpers'
...
export abstract class DynamoModel<T extends BaseModel> {
  ...
  constructor(tableName: string, region?: string, logger?: Logger) {
    ...
    this.logger = logger ||
      new Logger({
        serviceName: process.env['SERVICE_NAME'] || 'aws-starter-kit-api',
  ...
  async getById(id: string): Promise<T | null> {
    try {
      ...
      if (!response.Item) {
        console.log(`${this.getEntityName()} not found: ${id}`)
        return null
      }

      const entity = unmarshall(response.Item) as T
      console.log(`${this.getEntityName()} retrieved successfully: ${id}`)
      return this.cleanEntity(entity)
    } catch (error) {
      console.error(`Error retrieving ${this.getEntityName().toLowerCase()} ${id}:`, error)
  ...
  async create(entityData: Omit<T, keyof BaseModel>): Promise<T> {
    try {
      ...
      if (entity.id === undefined) {
        entity.id = await this.getIdWithPrefix(await this.generateId(entity))
      }
      else {
        entity.id = await this.getIdWithPrefix(entity.id)
      }
      ...
      console.log(`${this.getEntityName()} created successfully: ${entity.id}`)
      ...
      console.error(`Error creating ${this.getEntityName().toLowerCase()}:`, error)
  ...
  async update(id: string, updates: Partial<Omit<T, 'id' | 'createdAt'>>): Promise<T | null> {
    try {
      ...
      if (!response.Attributes) {
        console.log(`${this.getEntityName()} not found for update: ${id}`)
        return null
      }

      const updatedEntity = unmarshall(response.Attributes) as T
      console.log(`${this.getEntityName()} updated successfully: ${id}`)
      return updatedEntity
    } catch (error) {
      console.error(`Error updating ${this.getEntityName().toLowerCase()} ${id}:`, error)
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      await this.dynamoClient.send(command)
      console.log(`${this.getEntityName()} deleted successfully: ${id}`)
      return true
    } catch (error) {
      console.error(`Error deleting ${this.getEntityName().toLowerCase()} ${id}:`, error)
  ...
  async batchDelete(ids: string[]): Promise<{ success: string[]; failed: Array<{ id: string; error: string }> }> {
    try {
      ...
      for (const chunk of chunks) {
        ...
        try {
          ...
          if (response.UnprocessedItems && response.UnprocessedItems[this.tableName]) {
            response.UnprocessedItems[this.tableName].forEach((item: any, index: number) => {
          ...
          chunk.forEach(id => {
            results.failed.push({
              id,
              error: batchError.message || 'Batch write operation failed'
            })
            this.logger.error(`${this.getEntityName()} deletion failed: ${id}`, { error: batchError })
          })
        }
      }
      ...
      throw new Error(`Failed to batch delete ${this.getEntityName()}s: ${error.message}`)
    }
  }
   ...
   */
  async scanAll(filterExpression?: string, expressionAttributeValues?: Record<string, any>): Promise<T[]> {
    try {
      ...
      return entities.filter(entity => this.idHasPrefix(entity.id)).map(entity => this.cleanEntity(entity))
    } catch (error) {
      console.error(`Error scanning ${this.getEntityName().toLowerCase()}s:`, error)
  ...
  async queryByGSI(
    ...
    try {
      ...
      this.logger.info(`Retrieved ${entities.length} for ${this.getEntityName()}s by GSI: ${indexName}`, { entities })
      return entities.filter(entity => this.idHasPrefix(entity.id)).map(entity => this.cleanEntity(entity))
    } catch (error: any) {
      console.error(`Error querying ${this.getEntityName()}s by GSI:`, error)
      throw new Error(`Failed to query ${this.getEntityName()}s by GSI: ${error.message}`)
    }
  }
